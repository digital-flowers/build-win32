...
if test -x PerlMagick -a -f Makefile.aperl ; then
  ...
  ${MAKE} -f Makefile.aperl CC='i686-w64-mingw32-gcc-win32 -std=gnu99 -std=gnu99' TEST_VERBOSE=1 test
elif test -f Makefile -a -f Magick.o; then
  # Shared build test incantation
  ${MAKE} CC='i686-w64-mingw32-gcc-win32 -std=gnu99 -std=gnu99' TEST_VERBOSE=1 test
else
